...
export function exportPdf(submission: SubmissionDetail) {
  ...
  const typedResults = results as Record<string, OpponentResult> | null;
  let avgHtml = "";
  if (typedResults) {
    const accs = Object.entries(typedResults)
  ...
  const resultsTable = typedResults ? buildResultsTable(typedResults) : "";
