    ...
    existing = (
        (
            await session.execute(
                select(Submission).where(Submission.method_group == canonical_name).order_by(Submission.version.desc())
            )
        )
        .scalars()
        .all()
...
async def get_submission_versions(
    ...
    for sub in siblings:
        ...
        if job and job.results_json:
            ...
            accs = [
                v["avg_final_accuracy"]
                for v in data.values()
                if isinstance(v, dict) and v.get("avg_final_accuracy") is not None
            ]
            if accs:
